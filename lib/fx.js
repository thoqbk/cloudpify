...
Fx.getMethodNames = function (object) {
    ...
    var retVal = objectNMethodNamesMap[objectId];
    if (retVal == null) {
        retVal = [];
        ...
        //cache
        objectNMethodNamesMap[objectId] = retVal;
    }
    //return
    return retVal;
};
...
function extractParameters(fx) {
    ...
    var retVal = fxNParametersMap[fxId];
    if (retVal == null) {
        var retVal = [];
...
function extractAnnotations(scriptInString) {
    ...
    allMultiLineComments.forEach(function (multiLineComment) {
        var annotationPattern = /(\-\-)?\s*\@(\w+)\s*(\((\s*\w+\s*\=\s*\"[^"]+\"\s*\,?)*\))?/g;
        while (true) {
            ...
            var comment = annotationInStrings[1];
