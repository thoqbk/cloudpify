...
function Channel() {
    ...
    if (numberOfArgs == 3 && arguments[1].emit != null) {//could be socket.io
        userId = arguments[0];
        socketIOConnection = arguments[1];
        $logger = arguments[2];
        type = "socket.io";
        id = socketIOConnection.id;
    } else if (numberOfArgs == 4) {//could be expressjs
        userId = arguments[0];
    ...
    this.emit = function (stanza) {
        switch (type) {
            case "socket.io":
            {
                socketIOConnection.emit("cloudpify", stanza);
                break;
            }
            case "expressjs":
            {
        ...
        //debug
        $logger.debug("Finish emitting message: " + JSON.stringify(stanza));
    };
