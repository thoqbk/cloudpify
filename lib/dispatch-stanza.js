...
module.exports = function (userService, sessionService, $event, $logger, controllerLoader, $invoke) {
  ...
  function dispatchStanza(stanza, channel) {
    ...
    var $input = new Input(userId, stanza, channel);
    var $session = sessionService.get(userId);
    var $response = new Response(userService, sessionService, channel, stanza);

    Promise.resolve(beforeFx != null ? $invoke(beforeFx, null, buildActionArgumentsResolver($input, $session, $response)) : null)
    .then(function () {
      if (!$response.isEnded()) {
        return $invoke(actionFx, controller, buildActionArgumentsResolver($input, $session, $response));
      }
    })
    .then(function () {
      return afterFx != null ? $invoke(afterFx, null, buildActionArgumentsResolver($input, $session, $response)) : null;
    })
